...
import { createA2AClientHandler } from '../services/a2aclient';


function ChatInterface({ agentUrl }) {
  ...
  const [agentHandler, setAgentHandler] = useState(null);
  ...
  useEffect(() => {
    ...
    const handler = createA2AClientHandler(agentUrl);
  ...
  const handleFormSubmit = async (messageId, formData) => {
    ...
    abortStreamingRef.current = agentHandler.sendMessageStream(
      payload,
      (streamEvent) => {
